...
const { gray, blue, yellow } = ansiColors;
...
async function applyVersion(version: Version) {
	...
	await writeFile(join(__dirname, '..', 'package.json'), JSON.stringify(packageJson, null, '\t') + '\n');
	...
	for (const item of await readdir(packagesFolder, { withFileTypes: true })) {
		...
		await writeFile(join(packagePath, 'package.json'), JSON.stringify(subPackageJson, null, '\t') + '\n');
	}
